    ...
    def set_translation(self, translation, status):
        try:
            if not isinstance(translation, str):
                raise TypeError(f"Invalid translation type. Expected str, but got {type(translation)}")

            LOG.debug(f"[translation]\n{translation}")
             # 替换日语翻译时将顿号为逗号
            translation = translation.replace('、', ',')
            # 提取表头：先按']'分割字符串，取第一部分，并去除首个'['，然后按', '分隔
            header = translation.split(']')[0][1:].split(', ')
            # 提取数据行：先按'] '分割字符串，取剩余部分
            data_rows = translation.split('] ')[1:]
            # 处理每一行数据：去除行首尾的'['和']'，然后按中文逗号', '分割
